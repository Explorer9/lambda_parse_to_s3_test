    ...
    for child in element.children:
        if hasattr(child, 'name'):
            if child.name == 'a' and child.get('href'):
                ...
                result += extract_text_with_links(child)
        else:
            # It's a text node
            result += str(child)
...
def simple_parse(url):
    """Simplified version that just gets the main content with links"""
    ...
    # Get the answer content with links
    answer = soup.find('div', class_='regions-help-new-answer')
    answer_text = extract_text_with_links(answer) if answer else "No answer found"
    
    # Get table data (routing numbers) with links
